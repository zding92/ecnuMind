 ...
 * 限制个人能力描述输入字数为100字
 */
function selfCommentTextCounter(selector) {
	 $(selector).keyup(function detailCommentTextCount(){
		//如果当前输入字数超过100字，截断多余的字并弹窗。
		//否则显示当前剩余字数
		var abilitySelfCommentLength = $(selector).val().length;
		if(abilitySelfCommentLength > 100){
			var currentComment = $(selector).val().slice(0,100);
			$(selector).val(currentComment);
			myAlert("多余的字会被截断");
			$("#selfDetailTextCountNumber").text(0);
		}
		else {
			var now;
			if (selector == "#newAbilityDetail")
				now = "#selfDetailTextCountNumber_n";
			else
				now = "#selfDetailTextCountNumber_a";
			$(now).text(100 - $(selector).val().length);
		}
	});
}
	...
	$.ajax({
        ...
        success: function (result) {
        	...
        	if (newSelfComment == "") {
        		...
        		$("#selfDetailTextCountNumber_a").text(100 - selfComment.length);	
        	}
        	else {
        		$(".theme-popover .abilityDetail").val(newSelfComment);
        		//当点击能力标签且用户拥有该能力时，刷新selfComment的剩余字数
        		$("#selfDetailTextCountNumber_a").text(100 - newSelfComment.length);	
        	}
...
function editAbility() {
    $('.theme-popover .closure').click(function(){
	...
	$('.theme-popover .popoutLine1Save').click(function(){
        ...
        $.ajax({//将用户名以及最新点击的能力标签返回给后台，后台处理后，返回给前台此标签对应的selfComment，并显示
            ...
            success: function (result) {
            	switch (result) {
				case 'insert_success':
					var newAbility = {ability_name:lastPickedCheckbox};
					user_ability_json.push(newAbility);
					break;
				case 'update_success':
					break;
				case 'delete_success':
					var deleteAbilityIdx;
					for (deleteAbilityIdx in user_ability_json)
						if (user_ability_json[deleteAbilityIdx]['ability_name'] == lastPickedCheckbox) {
							break;
						}
					user_ability_json.splice(deleteAbilityIdx, 1);
					break;
				default:
					break;
				}
...
function recommendFrame(result, abilitySelfComment, judgeData) {
	...
	$('.similarAbilityGallery :input').each(function () {
	    ...
	    self.parent().addClass('abilityItemTag');
	    // 已经有的能力要勾选
	...
	selfCommentTextCounter("#abilityDetail");
    editAbility();
 ...
 * 在ability.js里调用
 */
function initAbilityAddEvent() {
	selfCommentTextCounter("#newAbilityDetail");
	$(".abilityAddBtn").click(function () {
		// 生成领域和方向下拉框
		addressInit('cmbProvince', 'cmbCity', 'cmbArea', '陕西', '宝鸡市', '金台区');
		$('#newAbilityDetail').val("");
		$('#selfDetailTextCountNumber_n').text(100);
		// 弹出新增能力框
		$('.theme-popover-mask').fadeIn(100);
